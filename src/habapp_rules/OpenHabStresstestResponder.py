...
class OpenHabStresstestResponder(HABApp.Rule):
    def __init__(self):
        super().__init__()

        # Eingangs-Topic für Stresstest (Publisher sendet hierhin)
        self.cmd = MqttItem.get_create_item("/stresstest/openhab/command")
    ...
    def on_message(self, event):
        ...
        try:
            ...
            self.mqtt.publish("/stresstest/openhab/response", json.dumps(response))
            log.info(f"Response gesendet für id={msg_id}")

        except Exception as e:
            log.error(f"Fehler im OpenHabStresstestResponder: {e}")
